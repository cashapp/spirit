...
func NewTableLock(ctx context.Context, db *sql.DB, table *table.TableInfo, lockWaitTimeout int, logger loggers.Advanced) (*TableLock, error) {
	lockTxn, _ := db.BeginTx(ctx, nil)
	_, err := lockTxn.ExecContext(ctx, "SET SESSION lock_wait_timeout = ?", lockWaitTimeout)
	if err != nil {
	...
	for i := 0; i < MaxRetries; i++ {
		...
		// has a brief race.
		logger.Warnf("trying to acquire table lock, timeout: %d", lockWaitTimeout)
