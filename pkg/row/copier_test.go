...
func TestCopier(t *testing.T) {
	...
	copier, err := NewCopier(pool, t1, t2, copierConfig)
	assert.NoError(t, err)
...
func TestThrottler(t *testing.T) {
	...
	copier, err := NewCopier(pool, t1, t2, NewCopierDefaultConfig())
	assert.NoError(t, err)
...
func TestCopierUniqueDestination(t *testing.T) {
	...
	copier, err := NewCopier(pool, t1, t2, cfg)
	assert.NoError(t, err)
...
func TestCopierValidation(t *testing.T) {
	...
	// if the checksum is FALSE, the unique violation will cause an error.
	_, err = NewCopier(pool, t1, nil, NewCopierDefaultConfig())
	assert.Error(t, err)
}
...
func TestETA(t *testing.T) {
	...
	copier1, err := NewCopier(pool, t1, t1new, NewCopierDefaultConfig())
	assert.NoError(t, err)
	copier2, err := NewCopier(pool, t2, t2new, NewCopierDefaultConfig())
	assert.NoError(t, err)
...
func TestCopierFromCheckpoint(t *testing.T) {
	...
	copier, err := NewCopierFromCheckpoint(pool, t1, t1new, NewCopierDefaultConfig(), lowWatermark, 3, 3)
	assert.NoError(t, err)
	assert.NoError(t, copier.Run(context.Background())) // works
