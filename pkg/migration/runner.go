...
func (m *Runner) setup(ctx context.Context) error {
	...
	if err := m.resumeFromCheckpoint(ctx); err != nil {
		...
		m.replClient = repl.NewClient(m.db, m.host, m.table, m.newTable, m.username, m.password, m.logger)
...
func (m *Runner) checksum(ctx context.Context) error {
	checker, err := checksum.NewChecker(m.db, m.table, m.newTable, m.replClient, &checksum.CheckerConfig{
